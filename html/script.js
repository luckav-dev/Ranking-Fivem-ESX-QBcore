const CONFIG = { maxRows: 50, defaultAvatar: 'https://cdn.discordapp.com/embed/avatars/0.png' };

let state = { isVisible: false, players: [], searchQuery: '' };
...
function renderRows() {
  ...
  filteredPlayers.forEach((player) => {
    ...
    const avatar = player.avatar || CONFIG.defaultAvatar;
    
    row.innerHTML = `
      <div class="rank ${rankClass}">#${player.rank}</div>
      <div class="player-info">
        <img src="${avatar}" alt="${player.name}" class="avatar" onerror="this.src='${CONFIG.defaultAvatar}'">
        <span class="username">${player.name}</span>
      </div>
      <div class="kda-stats">
        <div class="stat-item">
          <span class="stat-label">K</span>
          <span class="stat-val">${player.k}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">D</span>
          <span class="stat-val">${player.d}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">A</span>
          <span class="stat-val">${player.a}</span>
        </div>
        <div class="stat-item">
